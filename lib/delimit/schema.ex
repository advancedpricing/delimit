defmodule Delimit.Schema do
  ...
  * `:format` - Predefined format (`:csv`, `:tsv`, `:psv`) that sets appropriate options
  ...
  defp to_struct_with_prefix(%__MODULE__{} = schema, row, headers, prefix, _header_positions, opts) do
  ...
  # Find the position of this field in the row using the prefix
      ...
      col_idx = if is_list(headers) do
        ...
        Enum.find_index(headers, fn header -> 
          String.trim(String.downcase(header)) == String.trim(String.downcase(prefixed_field))
        end)
      else
  ...
  def to_row(%__MODULE__{} = schema, struct_or_map) do
    ...
    row_values = Enum.map(regular_fields, fn field ->
      value = Map.get(struct_or_map, field.name)
      Field.to_string(value, field)
    end)
    
    # Add embedded field values
    embedded_values = Enum.flat_map(embed_fields, fn embed_field ->
      ...
      if is_nil(embed_struct) do
        ...
        embedded_values = Enum.map(embed_fields, fn field ->
