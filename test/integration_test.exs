...
# Define test module outside the test
...
defmodule Delimit.IntegrationTest do
  ...
  describe "basic functionality" do
    ...
    test "read with custom options" do
      ...
      people = TestPerson.read_string(csv_with_comments, 
    ...
    test "custom read/write functions" do
      # Define the module outside the test
      csv_data = String.replace("""
      name,tags
  ...
  describe "file operations" do
    setup do
      ...
      test_file = Path.join(System.tmp_dir(), "delimit_test_#{:rand.uniform(1000000)}.csv")
      on_exit(fn -> 
        File.rm(test_file) 
    ...
    test "read and write files", %{test_file: test_file} do
      # Create test data
      people = [
        %{first_name: "John", last_name: "Doe", age: 30, salary: 50000.5, 
          hired_date: ~D[2020-01-15], active: true, notes: "Good employee"},
        %{first_name: "Jane", last_name: "Smith", age: 28, salary: 55000.75, 
          hired_date: ~D[2019-05-20], active: true, notes: nil}
      ]
    ...
    test "stream operations", %{test_file: test_file} do
      ...
      people = for i <- 1..20 do
        %{
          ...
          active: rem(i, 2) == 0,
          notes: if(rem(i, 3) == 0, do: "Note for #{i}", else: nil)
        }
      end
